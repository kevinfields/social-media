...
const MyFeed = (props) => {
  ...
  const getPosts = async () => {
    ...
    for (const f of friends) {
      await props.firestore.collection('users').doc(f).collection('posts').get().then((snap) => {
        snap.forEach((doc) => {
          data.push({
            data: doc.data(),
            id: doc.id,
    ...
    <div id='main-feed'>
      {
        posts.map((post) => (
          <section className='feed-post' key={post.id}>
            ...
            onLike={() => likePost(props.user, post.id, props.firestore.collection('users').doc(post.data.uid).collection('posts'))}
            userRef={userRef}
            commentRef={props.firestore.collection('users').doc(post.data.uid).collection('posts').doc(post.id).collection('comments')}
            /> 
          </section>
        ))
      }
